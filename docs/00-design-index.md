...
| `implementation/roadmap.md` | Technical-specification programme | S00–S06 documented | Complete S07–S14 before any implementation approval. |
| `implementation/status.md` | Durable current state | Current; S07 next | Update in every specification commit. |
| `implementation/decisions.md` | Technical and process decisions | Current through S06 | Add only approved decisions and preserve supersessions. |
| `implementation/interfaces.md` | Shared-interface lifecycle | `MR-IF-001`–`MR-IF-006` candidate; `MR-IF-011` S05 part draft; none frozen | Draft and freeze each interface in its owning block. |
| `implementation/open-issues.md` | Technical clarification ledger | `MR-IMP-OPEN-007` active | Resolve each stable issue group in its owning block. |
...
- Next technical-specification block: **S07 — Persistence and recovery**;
  Leonardo's approval to begin it is pending after the S06 commit.
- Last documented technical block: **S06 — Content data and build profiles**.
- Durable technical resume point: `implementation/status.md`.
- Active issue group: `MR-IMP-OPEN-007`.
- Technical-specification gate: **blocked**; S07–S14 remain.
