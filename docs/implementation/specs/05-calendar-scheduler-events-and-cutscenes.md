...
## 2026-09-08 — MR-IF-005 v3 Step5 amendment

MR-IMP-DEC-310 freezes the event/delivery/room-route unions and exact selected bindings in `analysis/step-05-content-contract.md` subsection20.2. `resolveRoomState` alone can create `MR-EVT-GABRIEL-QUEUE`. While it is queued, eligible or active, all three direct facility routes are unavailable with no state change. A committed Gabriel option applies its correlated scene route once. Expiry can apply only the owning LIMITED fallback while the room remains unresolved. A resolved room disables result and fallback. An already active scene can finish after its window under the existing rule.
