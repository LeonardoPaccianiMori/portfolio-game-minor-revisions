...
There is no active issue group. `MR-IMP-OPEN-017` preserves the complete
independent-review finding and is resolved by Leonardo's approved evidence-led
`MR-IF-002 v3` supersession. Dependent correction work can use only the exact
superseding work orders. A later material conflict must receive a new stable
