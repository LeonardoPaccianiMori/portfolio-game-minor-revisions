...
| MR-IMP-OPEN-021 | Step 5 content contract   | Exact executable content shapes, semantic registries, slice selection, phase commands and ownership                                                                                                                 | Resolved | `analysis/step-05-content-contract.md`; MR-IMP-DEC-310; frozen Step 5 successor interfaces                                         |
| MR-IMP-OPEN-022 | Step 6 fixture contract   | Exact phase-aware fixture schema, current schema/content baseline, reference data, executable case boundary and work order                                                                                          | Resolved | `analysis/step-06-fixture-contract.md`; MR-IMP-DEC-311; frozen `MR-IF-015 v2`                                                      |
...
MR-IMP-OPEN-022 is resolved by Leonardo's approval of MR-IMP-DEC-311 and frozen `MR-IF-015 v2`. MR-IMP-OPEN-020 remains open for later fallback content and does not block Step 6. Issues 001–019 and 021–022 preserve resolved history.
...
## MR-IMP-OPEN-021 — Step 5 executable content contract (resolved)

S06 names condition/effect types and family properties without a complete closed payload/target registry. Its default-full build rejection also needs reconciliation with current mandatory verification commands. Leonardo approved primary candidate preparation on2026-09-06. The current candidate record is analysis/step-05-content-contract.md; historical checkpoint735816b preserves the approved preparation plan. No worker may invent these shared contracts. Resolve the exact candidate and any literal slice gaps before dependent implementation. This issue is distinct from fallback-window020 and resolved npm019.
...
021 update: the initial independent high-level review is complete. The candidate now separates built/source shapes, corrects scene-completion ownership and Clarified closing, enumerates proposed runtime projections, and includes a concrete laser-result contract. D1 and D3 are approved by Leonardo; D4 is resolved from existing source. The remaining literal/trigger/closure decisions remain open. These newer candidate details have not frozen MR-IF-006 or authorized implementation.

2026-09-07,021 preparation: candidate sections18–19 now specify the finite laser outcome-row key, single command cost owner, narrower static-check claim, exact proposed titles/reasons/recaps and tutorial input/trigger names. These additions reduce unspecified authoring; they do not close the final selected manifest or the remaining full-packet approval gate.

2026-09-08,021 decision: Leonardo approved the complete D3 laser mapping in candidate section16. Strong, Limited and Weak meanings; observation/access/monitoring projections; honest-reading limits; fixed result text; and representative vectors are now approved inputs to the final amendment. D3 is closed. D2, D5, D6, D7, the final selected manifest/counts and the full amendment/implementation approval remain open.

Focused review found a concrete021 source conflict: tutorial.move/interact/status hard-code WASD/mouse, E and Tab, but S09 requires the current binding. Semantic input references alone do not fix the body text. Prepare neutral approved fixed text with separate current-binding presentation, or an exact equivalent rule, before implementation. The draft records this hold; review found no blocker to preserving the preparation checkpoint.

2026-09-08,021 closure candidate: section20 now supplies the exact D2/D5/D6/D7 proposal, binding-safe tutorial text, event/room/task/record meanings, 97 selected top-level IDs, 213 selected English keys, 144 laser rows, 29 interface items, 26 semantic audio roles, phase commands, amendment impact and file-level work orders. It excludes the unbacked laser report and break actions. The packet remains candidate. Primary audit and one fresh high-level review must complete before Leonardo receives the final amendment approval request. MR-IF-006 v2 and the other frozen authorities remain unchanged.

2026-09-08,021 complete-candidate review: the first fresh high-level review confirmed the literal counts and found eight contract groups. Primary corrected the opening event/state identity, strict milestone and record variants, recap union, room-event lifecycle and result routes, static input-binding boundary, complete draft order data, deferred execution ownership and fallback wording in section20.

2026-09-08,021 final preparation: a second fresh high-level review found four required groups. Primary closed the five-ID dormant-state exception, queued-event case, experiment text fields/views and S13 execution sequence. Focused re-check then found no blocker or required finding after the S03/S06 issue split, current room-route projections and later profile owners were explicit. The complete primary contract/count/JSON/scope/privacy/format/diff audit passed. Preparation is complete and021 now awaits Leonardo's one combined decision on D2/D5/D6/D7, the named interface/specification amendments and exact implementation plans. Current frozen authorities and Step5 code remain unchanged.

2026-09-08,021 resolution: Leonardo approved the complete reviewed packet. MR-IMP-DEC-310 and the frozen successor interfaces record D2/D5/D6/D7 and the exact specification and implementation authority. Step5 implementation is active. Runtime correctness, review, Leonardo testing and acceptance remain pending.

## MR-IMP-OPEN-022 — Step 6 fixture contract

Leonardo approved bounded preparation and correction of the reference-trace count from 12 to 13 on 2026-09-09. The candidate must define the exact `MR-IF-015 v2` fixture, manifest, acceptance-matrix, phased registration, current executable evidence, reference-only data, deferred routes, ownership, commands, review and acceptance boundaries. It must replace obsolete schema-1/content-1.0.0/full fixture assumptions with the accepted campaign schema `2`, content `1.1.0` and complete slice profile without inventing migration or complete full/fallback data.

The preparation candidate is `analysis/step-06-fixture-contract.md`, with implementation order in `plans/step-06-fixture-foundation.md`. It defines 103 executable wrappers, 48 reference-only wrappers, 41 deferred setups, 45 acceptance rows and four check-safe work orders. Primary count, source, example, ownership, scope, format and diff checks passed. High-level reviews found and closed the source-packet, evidence, deferred-route, pending-operation, evaluation-group and semantic-evidence gaps; the final focused result is PASS with no blocker or required finding. At preparation time, this record did not freeze `MR-IF-015 v2`, authorize fixture implementation, execute a future rule or accept Step 6. The resolution below records Leonardo's later approval.

### 2026-09-09 resolution

Leonardo approved the complete reviewed amendment and the four exact
implementation work orders. MR-IMP-DEC-311 and frozen `MR-IF-015 v2` resolve
the schema, baseline, reference-data, executable-case, evidence, ownership, and
activation questions. Implementation results, detailed review, Leonardo's
test, and Step 6 acceptance remain later evidence. `MR-IMP-OPEN-020` remains
the only open issue and does not block the Step 6 fixture foundation.
