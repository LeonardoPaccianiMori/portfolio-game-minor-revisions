...
through `specs/07-persistence-and-recovery.md`. `MR-IF-008` is candidate `v1`
through the connected S08 and S10 contracts. `MR-IF-009` is candidate `v1`
...
not claim that a machine-readable fixture or result exists. S13 now supplies
the exact future ownership, assignment, review, and integration boundary and
refines candidate `MR-IF-006` with explicit incomplete inactive development
profiles. S14 must complete the cross-interface audit before an applicable
